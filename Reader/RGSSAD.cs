...
public class RGSSAD : IDisposable
{
    ...
    public List<ArchivedFile> ArchivedFiles { get; set; } = new List<ArchivedFile>();
    ...
    /// <exception cref="Exception">Invalid file path. Archive could be corrupted.</exception>
    ...
    /// <returns></returns>
    public byte[] DecryptFileData(byte[] encryptedFileData, uint key)
    {
    ...
    public byte[] EncriptFileData(byte[] decryptedFileData, uint key)
    {
        byte[] encryptedFileData = new byte[decryptedFileData.Length];

        uint tempKey = key;
        byte[] keyBytes = BitConverter.GetBytes(key);
        int j = 0;

        for (int i = 0; i <= decryptedFileData.Length - 1; i++)
        {
            if (j == 4)
            {
                j = 0;
            ...
            encryptedFileData[i] = (byte)(decryptedFileData[i] ^ keyBytes[j]);
